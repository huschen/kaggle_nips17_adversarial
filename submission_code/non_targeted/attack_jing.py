...
tf.flags.DEFINE_string(
    'output_dir', '../../intermediate_results/attacks_output/jing',
...
def main(_):
  ...
  batch_shape = [FLG.batch_size, FLG.image_height, FLG.image_width, FLG.image_depth]
  ...
  eps, alpha = attack.parameters(max_epsilon, num_iter, image_factor,
  ...
  with tf.Graph().as_default():
    ...
    for model_name in model_names:
      with tf.variable_scope(model_name):
        ...
        target_class_input = tf.arg_min(logits, 1)
        pred_real_class = tf.cast(tf.arg_max(logits, 1), tf.int32)
        m_pred_real_class.append(pred_real_class)

    x_adv, _ = attack.adv_graph(x_input, target_class_input, x_ref,
                             m_predicted_labels, m_logits_weighted,
                             num_classes, alpha, eps,
                             m_real_class=m_pred_real_class)

    # logits_max = tf.reduce_max(logits, 1, keep_dims=True)
    # y = tf.to_float(tf.equal(logits, logits_max))
    # y may have multiple max, e.g. [0, 1, 1, 0], though unlikely
    # y_pred = y / tf.reduce_sum(y, 1, keep_dims=True)

    # use closest or furthurest??
    # target_class_input = tf.arg_max(logits * (1 - y), 1)
