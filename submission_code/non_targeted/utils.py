...
# untested
def load_real_class(input_dir):
  """Loads target classes."""
  with tf.gfile.Open(os.path.join(input_dir, 'real_class.csv')) as f:
    return {row[0] + '.png': int(row[1]) for row in csv.reader(f) if len(row) >= 2}
